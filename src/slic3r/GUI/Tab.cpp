...
/*#ifdef __WXOSX__
    ...
    m_undo_btn->Bind(wxEVT_LEFT_DOWN, ([this](auto &e) { on_roll_back_value(); }));
    m_undo_to_sys_btn->Bind(wxEVT_LEFT_DOWN, ([this](auto &e) { on_roll_back_value(true); }));
    ...
    //m_top_panel->SetBackgroundColour(wxColour(122, 122, 122));
    if (m_presets_choice)
...
#endif
    // tree
    m_tabctrl = new TabCtrl(panel->GetParent(), wxID_ANY, wxDefaultPosition, wxSize(20 * m_em_unit, -1),
        wxTR_NO_BUTTONS | wxTR_HIDE_ROOT | wxTR_SINGLE | wxTR_NO_LINES | wxWANTS_CHARS | wxTR_FULL_ROW_HIGHLIGHT);
...
/*#ifdef __WXOSX__
    ...
    m_btn_delete_preset->Bind(wxEVT_LEFT_DOWN, ([this](auto &e) { delete_preset(); }));
...
void Tab::add_border_hover_icon(wxWindow* parent, HoverBorderIcon** btn, const std::string& icon_name, const wxSize& icon_size)
{
    *btn = new HoverBorderIcon(parent, wxEmptyString, icon_name, wxDefaultPosition, icon_size, wxTE_PROCESS_ENTER);
    m_hover_border_icons.push_back(*btn);
}
...
Slic3r::GUI::PageShp Tab::add_options_page(const wxString& title, const std::string& icon, bool is_extruder_pages /*= false*/)
{
    ...
    //m_page_view->SetBackgroundColour(wxColor(0, 0, 255));
    ...
    while (cur_item >= 0) {
        ...
        for (auto page : m_pages)
        {
            ...
            m_tabctrl->SetItemTextColour(cur_item, clr == &m_modified_label_clr ? *clr : StateColor(
                        std::make_pair(wxColor("#CBCBCC"), (int)StateColor::NotChecked),
                        std::make_pair(wxColor("#000000"), (int) StateColor::Normal)));
            break;
        }
...
void Tab::update_changed_tree_ui()
{
    ...
    while (cur_item >= 0) {
        ...
        for (auto page : m_pages)
        {
            ...
            //m_sys_label_clr = wxColor(255, 0, 0);
...
void Tab::update_undo_buttons()
{
    ...
    m_undo_btn->        SetBitmap_(m_presets->get_edited_preset().is_dirty ? m_bmp_value_revert.name() : m_bmp_white_bullet.name());
...
void Tab::update_mode(const int mode)
{
    if (mode == -1) {
        m_mode = wxGetApp().get_mode();
    } else {
        m_mode = static_cast<ConfigOptionMode>(mode);
    }
...
void Tab::msw_rescale()
{
    ...
    for (const auto bmp : m_scaled_bitmaps)
        bmp->msw_rescale();
    for(const auto icon : m_hover_border_icons)
        icon->msw_rescale();
...
void Tab::on_value_change(const std::string& opt_key, const boost::any& value)
{
    ...
    if (opt_key == "overhang_optimization")
    {
        ...
        if (m_config->opt_bool("overhang_optimization"))
        {
            DynamicPrintConfig new_conf = *m_config;
            new_conf.set_key_value("overhang_optimization", new ConfigOptionBool(true));
            m_config_manipulation.apply(m_config, &new_conf);
            wxGetApp().plater()->update();
        }
        else
        {
            DynamicPrintConfig new_conf = *m_config;
            new_conf.set_key_value("overhang_optimization", new ConfigOptionBool(false));
            m_config_manipulation.apply(m_config, &new_conf);
            wxGetApp().plater()->update();
        }
    }
...
void Tab::on_presets_changed()
{
    ...
    wxGetApp().plater()->get_partplate_list().set_verder_type(vtp);
...
void TabPrint::build()
{
    ...
    optgroup->append_single_option_line("enable_support", "support");
        ...
        optgroup->append_single_option_line("ignore_inner_color");
        ...
        for (auto& page : m_pages) {
            for (auto& group : page->m_optgroups)
            {
                std::string icon = OptionConfig::Ins().get_group_icon(group->title.ToStdString());
                if(!icon.empty())
                    group->icon = icon;
            }
        }

        // rebind infill "sparse_infill_density" value changed to 100% event
        rebind_infill_changed_event();
...
/**
 * Rebinds the infill changed event to handle sparse infill density and pattern changes.
 *
 * @return None
 */
void TabPrint::rebind_infill_changed_event()
{
    PageShp strength_page = get_page(L("Strength"));
    if (strength_page) {
        ConfigOptionsGroupShp optgroup = strength_page->get_optgroup(L("Infill"));
        if (optgroup) {
            optgroup->m_on_change = [this, optgroup](t_config_option_key opt_key, boost::any value) {

                // auto infill_pattern_not_matched = [](const int& infill_pattern) -> bool {
                //     return (infill_pattern != InfillPattern::ipConcentric && infill_pattern != InfillPattern::ipRectilinear &&
                //             infill_pattern != InfillPattern::ipAlignedRectilinear && infill_pattern != InfillPattern::ipHilbertCurve &&
                //             infill_pattern != InfillPattern::ipArchimedeanChords && infill_pattern != InfillPattern::ipOctagramSpiral);

                wxString display_msg = (_L("sparse_infill_density_100"));

                if (!m_postpone_update_ui && (opt_key == "sparse_infill_density")) {
                    if (boost::any_cast<double>(value) >= 100) {
                        boost::any infill_pattern_value = optgroup->get_value("sparse_infill_pattern");
                        int        infill_pattern       = boost::any_cast<int>(infill_pattern_value);
                        if (InfillPattern::ipGrid == infill_pattern) {
                            
                            int response = InfoDialog(parent(), _L("warning"), display_msg, false, wxYES_NO).ShowModal();
                            if(response == wxID_YES) {
                                optgroup->set_value("sparse_infill_pattern", int(InfillPattern::ipRectilinear));
                            }
                            else {
                                optgroup->back_to_initial_value("sparse_infill_density");
                            }
                        }
                    }
                } else if (!m_postpone_update_ui && (opt_key == "sparse_infill_pattern")) {
                    int        infill_pattern        = boost::any_cast<int>(value);
                    boost::any sparse_infill_density = optgroup->get_value("sparse_infill_density");
                    double     infill_density        = boost::any_cast<double>(sparse_infill_density);
...
void TabPrint::toggle_options()
{
    ...
    Field* field2   = m_active_page->get_field("sparse_infill_pattern");
    if (auto choice2 = dynamic_cast<Choice*>(field2)) {
        ...
        if (!pre_is_same) {
            opt2.enum_values.clear();
            opt2.enum_labels.clear();
            if (cb2->icons_backup.size() < cb2->GetCount())
            {
                cb2->GetItemBitmaps(cb2->icons_backup);
            }
            cb2->Clear();
            for (auto i : set2) {
...
void TabFilament::updateVentorList(const std::unordered_set<wxString>& ventorList, const wxString& curPreset)
{
    ...
    //m_wcb_choiceVendor->SetSelection(curPreset);
    m_wcb_choiceVendor->SetStringSelection(curPreset);
}
...
void TabFilament::updatePrintList(const std::unordered_set<wxString>& printerList, const wxString& curPreset)
{
    ...
    //m_wcb_choicePrintType->SetSelection(curPreset);
...
void TabFilament::build()
{
    ...
    auto page = add_options_page(L("Filament"), "custom-gcode_filament"); // ORCA: icon only visible on placeholders
        ...
        line = {L("Customized Plate"), L("Bed temperature when cool plate is installed. Value 0 means the filament does not support to print on the Customized Plate")};
        line.append_option(optgroup->get_option("customized_plate_temp_initial_layer"));
        line.append_option(optgroup->get_option("customized_plate_temp"));
        optgroup->append_line(line);

        line = {L("Epoxy Resin Plate"), L("Bed temperature for layers except the initial one. Value 0 means the filament does not support to print on the Epoxy Resin Plate")};
        line.append_option(optgroup->get_option("epoxy_resin_plate_temp_initial_layer"));
        line.append_option(optgroup->get_option("epoxy_resin_plate_temp"));
        optgroup->append_line(line);
...
void TabFilament::toggle_options()
{
    ...
    if (m_active_page->title() == L("Cooling")) {
      ...
      toggle_option("enable_special_area_additional_cooling_fan", cfg.opt_bool("auxiliary_fan"));
    ...
    if (m_active_page->title() == L("Filament"))
    {
        ...
        toggle_line("epoxy_resin_plate_temp_initial_layer", support_multi_bed_types);
        toggle_line("customized_plate_temp_initial_layer", support_multi_bed_types);
...
// BBS: model config
    ...
    //StateColor::darkModeColorFor(wxColour(238, 238, 238)), wxDefaultPosition, wxSize(m_top_panel->GetSize().GetWidth(), 3 * wxGetApp().em_unit()), 8);
    auto search_sizer = new wxBoxSizer(wxHORIZONTAL);
    m_search_input = new TextInput(m_search_item, wxEmptyString, wxEmptyString, wxEmptyString, wxDefaultPosition, wxSize(FromDIP(200), FromDIP(30)), 0 | wxBORDER_NONE);
    m_search_input->SetBackgroundColour(wxColour(238, 238, 238));
    m_search_input->SetForegroundColour(wxColour(43, 52, 54));
    m_search_input->SetFont(wxGetApp().bold_font());
    ...
    if (!m_wcb_choiceVendor)
        m_wcb_choiceVendor = new TabPresetComboBox(m_boxPanel, Preset::TYPE_INVALID);
    m_wcb_choiceVendor->Bind(wxEVT_COMBOBOX, [this](wxCommandEvent& e) {
        dynamic_cast<ParamsPanel*>(GetParent())->setCurVentor(m_wcb_choiceVendor->GetValue().ToStdString());
        });
    m_wcb_choiceVendor->SetSize(240, 24);

    //m_top_sizer->Add(m_wcb_choiceVendor, 0, wxLEFT | wxRIGHT | wxALIGN_CENTER_VERTICAL, 1);
    bSizer->Add(m_wcb_choiceVendor);
    ...
    m_wcb_choicePrintType->Bind(wxEVT_COMBOBOX, [this](wxCommandEvent& e) {
        dynamic_cast<ParamsPanel*>(GetParent())->setCurType(m_wcb_choicePrintType->GetValue().ToStdString());
        });
    ...
    m_top_sizer->Add(m_search_item, 1, wxALIGN_CENTER_VERTICAL | wxRIGHT, 8);
    //m_search_item->Show();
    m_top_sizer->AddStretchSpacer(8);
    ...
    if (dynamic_cast<TabPrint*>(this) == nullptr) {
        ...
        m_mode_view = new SwitchButton(m_top_panel, wxID_ABOUT);
        //m_top_sizer->AddSpacer(4);
        m_top_sizer->Add(m_mode_view, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT, 8);
    }
    ...
    m_top_sizer->SetMinSize(-1, 3 * m_em_unit);
    m_top_panel->SetSizer(m_top_sizer);
    //m_top_panel->SetBackgroundColour(wxColor(255, 0, 0));
    if (m_presets_choice)
        m_main_sizer->Add(m_top_panel, 0, wxEXPAND | wxUP | wxDOWN, m_em_unit);
    else
        m_top_panel->Hide();
...
#endif
    // tree
    m_tabctrl = new TabCtrl(panel->GetParent(), wxID_ANY, wxDefaultPosition, wxSize(-1, -1),
...
void TabPrinter::extruders_count_changed(size_t extruders_count)
{
    ...
    ////add test  Orca: support multi tool
        ...
        val > 0 && (size_t)val <= m_extruders_count))
    {
        ...
        if (use_firmware_retraction && wipe) {
            ...
            if (dialog.ShowModal() == wxID_YES) {
                ...
                auto* opt = static_cast<ConfigOptionBool*>(m_config->option("use_firmware_retraction")->clone());
                opt->value = false;
                new_conf.set_key_value("use_firmware_retraction", opt);
            }
            CallAfter([=]() {
                load_config(new_conf);
             });
...
bool Tab::isCurrent_reset_system()
{
    const Preset& preset = m_presets->get_edited_preset();
    bool res = (m_type == Preset::TYPE_PRINTER && m_preset_bundle->physical_printers.has_selection())
...
//BBS: add project embedded preset relate logic
void Tab::save_preset(std::string name /*= ""*/, bool detach, bool save_to_project, bool from_input, std::string input_name, bool isSave/*= false*/)
{
...
//!	m_tabctrl->OnSetFocus();
    if (from_input) {
        SavePresetDialog dlg(m_parent, m_type, detach ? _u8L("Detached") : "", isSave);
        dlg.Show(false);
...
void Tab::delete_preset()
{
    ...
    if (current_preset.is_default || !(confirm_delete_third_party_printer ||
        ...
        wxID_YES == MessageDialog(parent(), msg, title, wxYES_NO | wxNO_DEFAULT).ShowModal()))
        return;
...
void TabPrinter::changedSelectPrint(size_t selection)
{
    if (!m_presets_choice->selection_is_changed_according_to_physical_printers()) {
        if (m_type == Preset::TYPE_PRINTER && !m_presets_choice->is_selected_physical_printer())
            m_preset_bundle->physical_printers.unselect_printer();
...
void TabPrinter::updateVentorList(const std::unordered_set<wxString>& ventorList, const wxString& curPreset)
{
    ...
    //m_wcb_choiceVendor->SetSelection(0);
    m_wcb_choiceVendor->SetStringSelection(curPreset);
}
...
void TabPrinter::updatePrintList(const std::unordered_set<wxString>& printerList, const wxString& curPreset)
{
    ...
    //m_wcb_choicePrintType->SetSelection(0);
    m_wcb_choicePrintType->SetStringSelection(curPreset);
}
...
void Tab::select_item(int item)
{
	int num = m_tabctrl->GetCount();
    if(item < num)
        m_tabctrl->SelectItem(item);
}

void Tab::select_item(wxString item)
{
    int index = 0;
	for (int i = 0; i < m_pages.size(); i++)
	{
        if (!m_pages[i]->get_show()) {
            index++;
            continue;
        }
...
bool Tab::get_page_is_modified_values(wxString item)
{
    for (int i = 0; i < m_pages.size(); i++) {
        if (!m_pages[i]->get_show())
            continue;
        ...
        wxString txt = m_pages[i]->title();
        if (txt == item) {
            return m_pages[i]->m_is_modified_values;
        }
    }
...
class TabbarPresetScrolledWindow : public wxScrolledWindow
{
public:
    TabbarPresetScrolledWindow(wxWindow* parent)
...
TabbarPresetCombox::TabbarPresetCombox(wxWindow* parent, const wxSize& size)
    : ::ComboBox(parent,
        wxID_ANY,
        wxEmptyString,
        wxDefaultPosition,
...
TabbarPresetCombox::~TabbarPresetCombox()
{
}

void TabbarPresetCombox::Clear()
{
    DoClear();
}

void* TabbarPresetCombox::GetClientData(unsigned int n) const
{
    return DoGetItemClientData(n);
}

HTabbarPrinter::HTabbarPrinter(ParamsPanel* parent, Preset::Type preset_type, PresetBundle* preset_bundle)
    : m_parent(parent), m_preset_bundle(preset_bundle)
{
    m_printer_collection = &preset_bundle->printers;
    m_filament_collection = &preset_bundle->filaments;

    Create(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBK_TOP | wxTAB_TRAVERSAL/*, name*/);
    this->SetFont(Slic3r::GUI::wxGetApp().normal_font());
    ...
    {
        Label* label_machine_type = new Label(this, _L("Printer: "), LB_PROPAGATE_MOUSE_EVENT);
        m_label_machine_type = new Label(this, _L("Printer"), LB_PROPAGATE_MOUSE_EVENT);
        wxBoxSizer* type_sizer = new wxBoxSizer(wxHORIZONTAL);
        type_sizer->Add(label_machine_type, 0, wxALIGN_CENTER);
        type_sizer->Add(m_label_machine_type, 0, wxALIGN_CENTER);
        m_machine_sizer->Add(type_sizer, 0, wxALIGN_LEFT);
        m_machine_sizer->AddSpacer(4);

        Label* label_machine_name = new Label(this, _L("Name: "), LB_PROPAGATE_MOUSE_EVENT);
        m_label_machine_name = new Label(this, _L("Name"), LB_PROPAGATE_MOUSE_EVENT);
        wxBoxSizer* name_sizer = new wxBoxSizer(wxHORIZONTAL);
        name_sizer->Add(label_machine_name, 0, wxALIGN_CENTER);
        name_sizer->Add(m_label_machine_name, 0, wxALIGN_CENTER);
        m_machine_sizer->Add(name_sizer, 0, wxALIGN_LEFT);
    }

    m_vendor_model_sizer = new wxBoxSizer(wxHORIZONTAL);
    {
        Label* label_vendor = new Label(this, _L("Vendor"), LB_PROPAGATE_MOUSE_EVENT);
        m_vendor_choice = new TabbarPresetCombox(this);
        m_vendor_model_sizer->Add(label_vendor, 0, wxALIGN_CENTER);
        m_vendor_model_sizer->AddSpacer(4);
        m_vendor_model_sizer->Add(m_vendor_choice, 1, wxLEFT | wxRIGHT | wxALIGN_CENTER_VERTICAL, 1);
        m_vendor_model_sizer->AddSpacer(10);

        Label* label_model = new Label(this, _L("Model"), LB_PROPAGATE_MOUSE_EVENT);
        m_model_choice = new TabbarPresetCombox(this);
        m_vendor_model_sizer->Add(label_model, 0, wxALIGN_CENTER);
        m_vendor_model_sizer->AddSpacer(4);
        m_vendor_model_sizer->Add(m_model_choice, 1, wxLEFT | wxRIGHT | wxALIGN_CENTER_VERTICAL, 1);
    }
    

    // search button
    m_search_button = new ScalableButton(this, wxID_ANY, "search", wxEmptyString, wxDefaultSize, wxDefaultPosition, wxBU_EXACTFIT | wxNO_BORDER, true);
    m_search_button->SetBackgroundColour(this->GetBackgroundColour());
    m_search_button->SetBitmapHovered_("search_hover");
    m_search_button->SetToolTip(_L("Search in preset"));
    // search box
    m_search_box = new StaticBox(this);
    StateColor box_colour(std::pair<wxColour, int>(*wxWHITE, StateColor::Normal));
    StateColor box_border_colour(std::pair<wxColour, int>(wxColour(238, 238, 238), StateColor::Normal));
    m_search_box->SetBackgroundColor(box_colour);
    m_search_box->SetBorderColor(box_border_colour);
    m_search_box->SetCornerRadius(5);
    // search input
    m_search_text_input = new TextInput(m_search_box, wxEmptyString, wxEmptyString, wxEmptyString, wxDefaultPosition, wxDefaultSize, 0 | wxBORDER_NONE);
    m_search_text_input->SetBackgroundColour(wxColour(238, 238, 238));
    m_search_text_input->SetForegroundColour(wxColour(43, 52, 54));
    m_search_text_input->SetFont(wxGetApp().bold_font());

    auto search_sizer = new wxBoxSizer(wxHORIZONTAL);
    search_sizer->Add(new wxWindow(m_search_box, wxID_ANY, wxDefaultPosition, wxSize(0, 0)), 0, wxEXPAND | wxLEFT | wxRIGHT, FromDIP(6));
    search_sizer->Add(m_search_text_input, 1, wxEXPAND | wxALL, FromDIP(2));

    m_search_box->SetSizer(search_sizer);
    m_search_box->Layout();
    search_sizer->Fit(m_search_box);
    m_search_box->Hide();

    // mode view
    m_mode_title = new Label(this, Label::Body_12, _L("Advance"));
    m_mode_title->Wrap(-1);
    // BBS: open this tab by select first
    m_mode_title->Bind(wxEVT_LEFT_UP, [this](auto&)
        {
            Tab* tab = dynamic_cast<Tab*>(m_parent->get_current_tab());
            tab->restore_last_select_item();
        });

    m_mode_view = new SwitchButton(this, wxID_ABOUT);

    wxBoxSizer* sizer = new wxBoxSizer(wxHORIZONTAL);

    sizer->AddSpacer(10);
    sizer->Add(m_machine_sizer, 0, wxALIGN_CENTER);
    sizer->Add(m_vendor_model_sizer, 1, wxALIGN_CENTER);
    sizer->AddSpacer(10);

    sizer->Add(m_search_button, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, 8);
    sizer->Add(m_search_box, 1, wxALIGN_CENTER_VERTICAL | wxLEFT, 8);
    sizer->AddSpacer(4);

    sizer->Add(m_mode_title, 0, wxALIGN_CENTER_VERTICAL | wxLEFT, 8);
    sizer->AddSpacer(4);
    sizer->Add(m_mode_view, 0, wxALIGN_CENTER_VERTICAL);
    sizer->AddSpacer(10);

    SetSizer(sizer);

    m_vendor_choice->Bind(wxEVT_COMBOBOX, [this](wxCommandEvent& e)
        {
            void* vendor_data = m_vendor_choice->GetClientData(m_vendor_choice->GetSelection());
            std::string* vendor = static_cast<std::string*>(vendor_data);
            if (vendor)
            {
                m_parent->OnSelectPrinterVendor(*vendor);
            }
            else
            {
                void* model_data = m_model_choice->GetClientData(m_model_choice->GetSelection());
                ProfileModel* model = static_cast<ProfileModel*>(model_data);
                if (model)
                {
                    m_parent->OnSelectPrinterWildVendor();
                }
                else
                {
                    m_parent->OnSelectPrinterWildVendorModel();
                }
            }
        });

    m_model_choice->Bind(wxEVT_COMBOBOX, [this](wxCommandEvent& e)
        {
            void* model_data = m_model_choice->GetClientData(m_model_choice->GetSelection());
            ProfileModel* model = static_cast<ProfileModel*>(model_data);
            if (model)
            {
                m_parent->OnSelectPrinterVendorModel(model->vendor, model->model);
            }
            else
            {
                void* vendor_data = m_vendor_choice->GetClientData(m_vendor_choice->GetSelection());
                ...
                if (vendor)
                {
                    m_parent->OnSelectPrinterWildModel(*vendor);
                }
                else
                {
                    m_parent->OnSelectPrinterWildVendorModel();
    ...
    m_search_box->Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent& e)
        {
            m_search_text_input->SetFocus();
        });

    m_search_text_input->Bind(wxCUSTOMEVT_EXIT_SEARCH, [this](wxCommandEvent&) 
        {
        Freeze();
    ...
    m_search_button->Bind(wxEVT_BUTTON,[this](wxCommandEvent&) 
        {
            Freeze();
            ...
            wxGetApp().plater()->search(false, tab->type(), this->GetParent(), m_search_text_input, m_search_button);
            Thaw();
        });
}

static std::string Loc_GetModel(const Preset& preset)
{
    return preset.config.opt_string("printer_model");
}

static std::string Loc_GetVendor(const Preset& preset)
{
    if (preset.vendor)
...
void HTabbarPrinter::ShowPrinter(bool show)
{
    m_vendor_model_sizer->Show(show);
}

void HTabbarPrinter::ShowFilament(bool show)
{
    m_machine_sizer->Show(show);
}

void HTabbarPrinter::SetMahineType(const wxString& type)
{
    m_label_machine_type->SetLabel(type);
}

void HTabbarPrinter::SetMahineName(const wxString& name)
{
    m_label_machine_name->SetLabel(name);
}

void HTabbarPrinter::OnUpdateVendor(const std::map<std::string, ProfileMachine>& machines, const std::string& vendor)
{
    m_vendor_choice->Clear();

    for (auto& iter : machines)
    {
        const std::string* name = &iter.first;
        int index = m_vendor_choice->Append(*name, wxNullBitmap, const_cast<std::string*>(name));
        if (*name != vendor)
            continue;
        m_vendor_choice->Select(index);
    }
    {
        m_vendor_choice->Append(_L("All"), wxNullBitmap, nullptr);
    }
}

void HTabbarPrinter::OnUpdateModel(const std::vector<const ProfileMachine*> machines, const std::string& model)
{
    m_model_choice->Clear();
    ...
    for (auto& machine : machines)
    {
        ...
        for (auto& profile_model : machine->models)
        {
            int index = m_model_choice->Append(profile_model.model, wxNullBitmap, const_cast<ProfileModel*>(&profile_model));
            if (!model.empty() && profile_model.model != model)
                continue;
            m_model_choice->Select(index);
        }
    }
    {
        int index = m_model_choice->Append(_L("All"), wxNullBitmap, nullptr);
        if(model.empty())
            m_model_choice->Select(index);
    }

    SwitchButton* mode_view = dynamic_cast<Tab*>(m_parent->get_current_tab())->m_mode_view;
    m_mode_view->SetValue(mode_view->GetValue());
}

class DataViewTextRenderer : public wxDataViewCustomRenderer
{
public:
    DataViewTextRenderer(std::function<void()> func)
        : wxDataViewCustomRenderer(GetDefaultType(), wxDATAVIEW_CELL_ACTIVATABLE), m_func(func)
    {
    }
    ...
    bool LeftClick(wxPoint cursor, wxRect cell, wxDataViewModel* model, const wxDataViewItem& item, unsigned int col) override
    {
...
TabbarDataViewModel::~TabbarDataViewModel()
{

}

unsigned int TabbarDataViewModel::GetColumnCount() const
{
    return 2;
}

wxString TabbarDataViewModel::GetColumnType(unsigned int col) const
{
    if (col == 0)
    {
        return wxT("string");
...
wxString TabbarDataViewModel::GetText(const wxDataViewItem& item) const
{
    if (!item.IsOk())
        return wxString();

    TabbarDataViewModelNode* node = static_cast<TabbarDataViewModelNode*>(item.GetID());
    return node->text;
}

std::string TabbarDataViewModel::GetName(const wxDataViewItem& item) const
{
    if (!item.IsOk())
        return std::string();
...
void TabbarDataViewModel::GetValue(wxVariant& variant, const wxDataViewItem& item, unsigned int col) const
{
    TabbarDataViewModelNode* node = static_cast<TabbarDataViewModelNode*>(item.GetID());
    if (col == 0)
    {
...
bool TabbarDataViewModel::SetValue(const wxVariant& variant, const wxDataViewItem& item, unsigned int col)
{
    if (col == 0)
    {
        TabbarDataViewModelNode* node = static_cast<TabbarDataViewModelNode*>(item.GetID());
...
wxDataViewItem TabbarDataViewModel::GetParent(const wxDataViewItem& item) const
{
    if (!item.IsOk())
        return wxDataViewItem(nullptr);

    TabbarDataViewModelNode* node = static_cast<TabbarDataViewModelNode*>(item.GetID());
    return wxDataViewItem(const_cast<TabbarDataViewModelNode*>(node->parent));
}

bool TabbarDataViewModel::IsContainer(const wxDataViewItem& item) const
{
    if (!item.IsOk())
        return true;
...
unsigned int TabbarDataViewModel::GetChildren(const wxDataViewItem& item, wxDataViewItemArray& children) const
{
    if (!item.IsOk())
    {
        for (auto& root : m_roots)
        {
            children.Add(wxDataViewItem((void*)&m_nodes.at(root)));
        }
        return m_roots.size();
    }

    TabbarDataViewModelNode* node = static_cast<TabbarDataViewModelNode*>(item.GetID());
    for (TabbarDataViewModelNode* child : node->children)
    {
        children.Add(wxDataViewItem(const_cast<TabbarDataViewModelNode*>(child)));
    }

    return node->children.size();
}

wxDataViewItem TabbarDataViewModel::AddRoot(const wxString& name)
{
    return _AddChild(0, name, name.ToStdString());
}

wxDataViewItem TabbarDataViewModel::AddChild(const wxDataViewItem& parent, const wxString& child, const std::string& name)
{
    if (!parent.IsOk())
    {
        return AddRoot(child);
    }

    TabbarDataViewModelNode* parent_node = static_cast<TabbarDataViewModelNode*>(parent.GetID());
    return _AddChild(parent_node->index, child, name);
}

wxDataViewItem TabbarDataViewModel::_AddChild(int parent, const wxString& child, const std::string& name)
{
    if (parent == 0 || m_nodes.count(parent) > 0)
    {
        std::pair<wxDataViewItem, wxDataViewItem> nodes = _CreateNode(parent, child, name);
        bool error = ItemAdded(nodes.first, nodes.second);
        return nodes.second;
    }
    else
    {
...
std::pair<wxDataViewItem, wxDataViewItem> TabbarDataViewModel::_CreateNode(int parent, const wxString& child, const std::string& name)
{
    int index = m_next_index++;
    m_nodes[index].index = index;
    m_nodes[index].available = false;
    m_nodes[index].text = child;
    m_nodes[index].name = name;
    if (parent == 0)
    {
        m_nodes[index].parent = nullptr;
        m_nodes[index].is_container = true;
        m_roots.emplace_back(index);
        return std::pair<wxDataViewItem, wxDataViewItem>(wxDataViewItem(nullptr), wxDataViewItem(&m_nodes.at(index)));
    }
    else
    {
        m_nodes[index].parent = &m_nodes.at(parent);
        m_nodes[index].is_container = false;
        m_nodes.at(parent).is_container = true;
        m_nodes.at(parent).children.emplace_back(&m_nodes[index]);
        return std::pair<wxDataViewItem, wxDataViewItem>(wxDataViewItem(&m_nodes.at(parent)), wxDataViewItem(&m_nodes.at(index)));
    }
}

void TabbarDataViewModel::DeleteChildren(const wxDataViewItem& parent)
{
    if (!parent.IsOk())
    {
        assert(0);
    ...
    {
        TabbarDataViewModelNode* parent_node = static_cast<TabbarDataViewModelNode*>(parent.GetID());
        std::deque<TabbarDataViewModelNode*> nodes_with_children;
        std::vector<TabbarDataViewModelNode*> nodes_to_delete;
        if (parent_node->children.size() > 0)
        {
            nodes_with_children.emplace_back(parent_node);
        }
        while (nodes_with_children.size() > 0)
        {
            TabbarDataViewModelNode* node = nodes_with_children.front();
            nodes_to_delete.emplace_back(node);
            nodes_with_children.pop_front();
            for (auto child : node->children)
            {
                if (child->children.empty())
                    continue;
                nodes_with_children.emplace_back(child);
            }
        }
        for (int i = 0; i < nodes_to_delete.size(); ++i)
        {
            TabbarDataViewModelNode* node = nodes_to_delete[nodes_to_delete.size() - 1 - i];
            wxDataViewItemArray children;
            for (auto child : node->children)
            {
                children.Add(wxDataViewItem(child));
            }
            ItemsDeleted(wxDataViewItem(node), children);
            for (auto child : node->children)
            {
                m_nodes.erase(child->index);
            }
        }
...
void TabbarDataViewModel::SetAvailable(const wxDataViewItem& item)
{
    if (!item.IsOk())
        return;
...
bool TabbarDataViewModel::GetAttr(const wxDataViewItem& item, unsigned int col, wxDataViewItemAttr& attr) const
{
    if (!item.IsOk())
        return false;
...
VTabbarPrinter::VTabbarPrinter(ParamsPanel* parent, Preset::Type preset_type, PresetBundle* preset_bundle)
    : m_parent(parent), m_preset_bundle(preset_bundle)
{
    {
        m_menu = new wxMenu;
        append_menu_item(m_menu, wxID_ANY, _L("Rename"), _L("Rename"),
            [this](wxCommandEvent&)
            {
                int a = 0;
            }, "", nullptr,
        ...
        append_menu_item(m_menu, wxID_ANY, _L("Delete"), _L("Delete"),
            [this](wxCommandEvent&)
            {
                int a = 0;
            }, "", nullptr,
    ...
    {
        TabbarDataViewModel* model = new TabbarDataViewModel();
        m_priner_view = new wxDataViewCtrl(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxDV_NO_HEADER | wxBORDER_SIMPLE);
        m_priner_view->AssociateModel(model);

        wxDataViewTextRenderer* text_renderer = new wxDataViewTextRenderer();
        DataViewTextRenderer* more_renderer = new DataViewTextRenderer([this]()
            {
                this->PopupMenu(m_menu);
            });
        wxDataViewColumn* name_col = new wxDataViewColumn(_L("Name"), text_renderer, 0, wxCOL_WIDTH_AUTOSIZE);
        wxDataViewColumn* more_col = new wxDataViewColumn(_L("More"), more_renderer, 1, wxCOL_WIDTH_AUTOSIZE);
        bool err1 = m_priner_view->AppendColumn(name_col);
        bool err2 = m_priner_view->AppendColumn(more_col);
        ...
        m_priner_view->Bind(wxEVT_DATAVIEW_SELECTION_CHANGED, [this](wxCommandEvent& evt)
            {
                wxDataViewCtrl* vendor_view = dynamic_cast<wxDataViewCtrl*>(evt.GetEventObject());
...
//                int index = vendor_view->GetColumnIndex(vendor_view->GetCurrentColumn());
                m_parent->OnSelectName(model->GetName(item));
            });


        m_priner_view->Bind(wxEVT_DATAVIEW_SELECTION_CHANGED, [this](wxDataViewEvent& evt)
            {
                int a = 0;
            });
    }
...
void VTabbarPrinter::_OnUpdateName(wxDataViewItem root, const std::set<std::string>& names, const std::string& selected_name, const std::unordered_set<std::string>& presets)
{
    ...
    for (auto& name : names)
    {
        ...
        {
            model->SetAvailable(item);
        }
    }
    m_priner_view->Expand(root);
}

TabbarCtrlPrinter::TabbarCtrlPrinter(ParamsPanel* parent, HTabbarPrinter* h_tabbar, VTabbarPrinter* v_tabbar, PresetBundle* preset_bundle)
    : m_parent(parent), m_h_tabbar(h_tabbar), m_v_tabbar(v_tabbar)
{
    m_printer_collection = &preset_bundle->printers;
}

void TabbarCtrlPrinter::OnInit(Preset& preset)
{
    _SetPrimitivePreset(preset);
    _SetCurrentPreset(&preset);

    std::set<std::string> vendors;
    std::set<std::string> models;
    std::set<std::string> user_names;
    std::set<std::string> sys_names;
    const std::deque<Preset>& presets = m_printer_collection->get_presets();
    const std::map<std::string, ProfileMachine>&  profile_machines = wxGetApp().app_config->get_profile_machines();
    
    m_system_machines = &profile_machines;
    const ProfileMachine* machine = nullptr;

    std::string preset_vendor = Loc_GetVendor(preset);
    std::string preset_model = Loc_GetModel(preset);
    for (auto& iter : profile_machines)
    {
        const ProfileMachine& profile_machine = iter.second;

        vendors.emplace(profile_machine.vendor);
        
        if (profile_machine.vendor != preset_vendor)
            continue;

        machine = &profile_machine;

        for (auto& profile_model : profile_machine.models)
        {
            models.emplace(profile_model.model);

            if (profile_model.model != preset_model)
                continue;

            for (auto& profile_printer : profile_model.printers)
            {
                sys_names.emplace(profile_printer.name);
            }
        }
    }

    for (int i = presets.front().is_visible ? 0 : m_printer_collection->num_default_presets(); i < presets.size(); ++i)
    {
        if(!presets[i].is_visible || !presets[i].is_compatible)
            continue;

        if (presets[i].is_user())
        {
            std::string vendor = Loc_GetVendor(presets[i]);
            std::string model = Loc_GetModel(presets[i]);
            if (preset_vendor != vendor || preset_model != model)
                continue;

            user_names.emplace(presets[i].name);
        }

        m_presets.emplace(presets[i].name);
    }
    m_h_tabbar->ShowPrinter(true);
    m_h_tabbar->ShowFilament(false);
    m_h_tabbar->OnUpdateVendor(*m_system_machines, preset_vendor);
    m_h_tabbar->OnUpdateModel({ machine }, preset_model);
...
void TabbarCtrlPrinter::OnExit()
{
    //std::string preset_name = _GetPrimitivePreset();
    //if (m_current_preset->name != preset_name)
    //{
    //    OnSelectName(preset_name);
    //}
}

void TabbarCtrlPrinter::OnSelectName(const wxString& name)
{
    Tab* tab = dynamic_cast<Tab*>(m_parent->get_current_tab());
    Preset* preset = tab->get_presets()->find_preset(name.ToStdString());
    if (!preset || !preset->is_visible || !preset->is_compatible)
    {
        m_parent->ShowDetialView(false);
        return;
    }
    ...
    _ResetCurrentPreset();
    _SetCurrentPreset(preset);
    tab->select_preset(Preset::remove_suffix_modified(preset->name));
}

void TabbarCtrlPrinter::OnSelectWildVendorModel()
{
    std::set<std::string> user_names;
    std::set<std::string> sys_names;
    const std::deque<Preset>& presets = m_printer_collection->get_presets();

    for (auto& iter : *m_system_machines)
    {
        const ProfileMachine& profile_machine = iter.second;
        for (int i = 0; i < profile_machine.models.size(); ++i)
        {
            for (auto& profile_printer : profile_machine.models[i].printers)
            {
                assert(!profile_printer.name.empty());
                sys_names.emplace(profile_printer.name);
            }
        }
    }
    ...
    for (auto& preset : presets)
    {
        ...
        user_names.emplace(preset.name);
    }

    std::vector<const ProfileMachine*> machines;
    for (auto& iter : *m_system_machines)
    {
        machines.emplace_back(&iter.second);
    }
    m_h_tabbar->OnUpdateModel(machines, std::string());
    m_v_tabbar->OnUpdateName(user_names, sys_names, *sys_names.begin(), m_presets);
    OnSelectName(*sys_names.begin());
}
void TabbarCtrlPrinter::OnSelectWildVendor()
{
    std::set<std::string> user_names;
    std::set<std::string> sys_names;
    const std::deque<Preset>& presets = m_printer_collection->get_presets();

    const ProfileMachine& profile_machine = m_system_machines->begin()->second;
    std::string vendor = profile_machine.vendor;
    std::string model;
    for (int i = 0; i < profile_machine.models.size(); ++i)
    {
        if (!model.empty())
    ...
    for (auto& preset : presets)
    {
        ...
        std::string preset_vendor = Loc_GetVendor(preset);
        std::string preset_model = Loc_GetModel(preset);
        if (preset_model == model && preset_vendor == vendor)
        {
            user_names.emplace(preset.name);
        }
    }
    ...
    m_h_tabbar->OnUpdateModel(machines, model);
    m_v_tabbar->OnUpdateName(user_names, sys_names, *sys_names.begin(), m_presets);
    OnSelectName(*sys_names.begin());
}
void TabbarCtrlPrinter::OnSelectWildModel(const wxString& vendor)
{
    std::set<std::string> user_names;
    std::set<std::string> sys_names;
    const std::deque<Preset>& presets = m_printer_collection->get_presets();

    const ProfileMachine& profile_machine = m_system_machines->at(vendor.ToStdString());
    for (auto& profile_model : profile_machine.models)
    {
        for (auto& profile_printer : profile_model.printers)
        {
...
void TabbarCtrlPrinter::OnSelectVendor(const wxString& vendor)
{
    std::set<std::string> user_names;
    std::set<std::string> sys_names;
    const std::deque<Preset>& presets = m_printer_collection->get_presets();
    ...
    m_h_tabbar->OnUpdateModel({ &profile_machine }, model);
    m_v_tabbar->OnUpdateName(user_names, sys_names, *sys_names.begin(), m_presets);
    OnSelectName(*sys_names.begin());
}

void TabbarCtrlPrinter::OnSelectVendorModel(const wxString& vendor, const wxString& model)
{
    std::set<std::string> user_names;
    std::set<std::string> sys_names;
    const std::deque<Preset>& presets = m_printer_collection->get_presets();
    ...
    for (auto& profile_model : profile_machine.models)
    {
        if (profile_model.model != model.ToStdString())
            continue;
...
void TabbarCtrlPrinter::_SetPrimitivePreset(const Preset& preset)
{
    m_primitive_preset_name = preset.name;
}

std::string TabbarCtrlPrinter::_GetPrimitivePreset() const
{
    return m_primitive_preset_name;
}

void TabbarCtrlPrinter::_SetCurrentPreset(Preset* preset)
{
    m_current_preset_visible = preset->is_visible;
    preset->is_visible = true;
    m_current_preset = preset;
}

void TabbarCtrlPrinter::_ResetCurrentPreset()
{
    if (!m_current_preset)
        return;

    m_current_preset->is_visible = m_current_preset_visible;
    m_current_preset = nullptr;
}

TabbarCtrlFilament::TabbarCtrlFilament(ParamsPanel* parent, HTabbarPrinter* h_tabbar, VTabbarPrinter* v_tabbar, PresetBundle* preset_bundle)
    : m_parent(parent), m_h_tabbar(h_tabbar), m_v_tabbar(v_tabbar)
{
    m_printer_collection = &preset_bundle->printers;
    m_filament_collection = &preset_bundle->filaments;
...
void TabbarCtrlFilament::OnInit(Preset& preset)
{
    _SetPrimitivePreset(preset);
    _SetCurrentPreset(&preset);

    Tab* tab_printer = dynamic_cast<Tab*>(m_parent->printer_panel());
    Preset& preset_printer = tab_printer->get_presets()->get_edited_preset();
    std::string vendor = Loc_GetVendor(preset_printer);
    std::string model = Loc_GetModel(preset_printer);
    ...
    for (size_t i = presets.front().is_visible ? 0 : m_filament_collection->num_default_presets(); i < presets.size(); ++i)
    {
        ...
        {
            sys_names.emplace(presets[i].name);
        }
        else if (presets[i].is_user())
        {
            user_names.emplace(presets[i].name);
        }
    }

    m_h_tabbar->ShowPrinter(false);
    m_h_tabbar->ShowFilament(true);
    m_h_tabbar->SetMahineType(_L(model));
...
void TabbarCtrlFilament::OnExit()
{
    //std::string preset_name = _GetPrimitivePreset();
    //if (m_current_preset->name != preset_name)
    //{
...
void TabbarCtrlFilament::OnSelectVendor(const wxString& vendor)
{
    // nothing to do
}

void TabbarCtrlFilament::OnSelectModel(const wxString& model)
{
    // nothing to do
}

void TabbarCtrlFilament::OnSelectName(const wxString& name)
{
    Tab* tab = dynamic_cast<Tab*>(m_parent->get_current_tab());
    Preset* preset = tab->get_presets()->find_preset(name.ToStdString());
    if (!preset || !preset->is_visible || !preset->is_compatible)
...
void TabbarCtrlFilament::_SetPrimitivePreset(const Preset& preset)
{
    m_primitive_preset_name = preset.name;
}

std::string TabbarCtrlFilament::_GetPrimitivePreset() const
{
    return m_primitive_preset_name;
}

void TabbarCtrlFilament::_SetCurrentPreset(Preset* preset)
{
    m_current_preset_visible = preset->is_visible;
    preset->is_visible = true;
    m_current_preset = preset;
...
void TabbarCtrlFilament::_ResetCurrentPreset()
{
    if (!m_current_preset)
        return;
