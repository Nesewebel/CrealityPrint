...
std::map<std::string, std::vector<SimpleSettingData>>  SettingsFactory::OBJECT_CATEGORY_SETTINGS=
{
    ...
    { L("Support"), {{"brim_type", "",1},{"brim_width", "",2},{"brim_object_gap", "",3},
                    ...
                    {"tree_support_brim_width", "",26}, {"tree_support_branch_angle", "",10},{"tree_support_branch_angle_organic","",10}, {"tree_support_wall_count", "",11},//tree support
                            ...
                            {"support_object_first_layer_gap", "", 28}, {"tree_support_wall_count_tree", "",29}, {"support_base_pattern_tree", "",30}, {"support_interface_min_area", "",31}
                            }},
...
wxMenu* MenuFactory::append_submenu_add_creality_model(wxMenu* menu, ModelVolumeType type) {
    auto sub_menu = new wxMenu;

    for (auto &item : {L("Cube"), L("3DBenchy"), L("Autodesk FDM Test"),
                       L("Voron Cube"), L("Stanford Bunny"), L("String Hell") }) {
        append_menu_item(
            ...
            [type, item](wxCommandEvent&) {
                ...
                else if (file_name == L("String Hell")) {
                    file_name     = "Orca_stringhell.3mf";
                    is_stringhell = true;
                } else
                    return;
                input_files.push_back((boost::filesystem::path(Slic3r::resources_dir()) / "creality_models" / file_name));
...
wxMenu* MenuFactory::append_submenu_add_testing_model(wxMenu* menu, ModelVolumeType type) 
{
    auto sub_menu = new wxMenu;

    if (type != ModelVolumeType::INVALID) {
        append_menu_item(
            sub_menu, wxID_ANY, _L("Load..."), "", [type](wxCommandEvent&) { obj_list()->load_subobject(type); }, "menu_load", menu);
        sub_menu->AppendSeparator();
    }

    append_menu_item(
        sub_menu, wxID_ANY, _L("Cube"), "", [type](wxCommandEvent&) { obj_list()->load_generic_subobject(L("Cube"),  type); },
        "menu_obj_cube", menu);

    append_menu_item(
        sub_menu, wxID_ANY, _L("Sphere"), "", [type](wxCommandEvent&) { obj_list()->load_generic_subobject(L("Sphere"), type); },
        "menu_obj_sphere", menu);

    append_menu_item(
        sub_menu, wxID_ANY, _L("Cylinder"), "", [type](wxCommandEvent&) { obj_list()->load_generic_subobject(L("Cylinder"), type); },
        "menu_obj_cylinder", menu);

    append_menu_item(
        sub_menu, wxID_ANY, _L("Cone"), "", [type](wxCommandEvent&) { obj_list()->load_generic_subobject(L("Cone"), type); },
        "menu_obj_cone", menu);

    append_menu_item(
        sub_menu, wxID_ANY, _L("Truncated Cone"), "", [type](wxCommandEvent&) { obj_list()->load_generic_subobject(L("Truncated Cone"), type); },
        "menu_obj_truncated_cone", menu);

    append_menu_item(
        sub_menu, wxID_ANY, _L("Torus"), "", [type](wxCommandEvent&) { obj_list()->load_generic_subobject(L("Torus"), type); },
        "menu_obj_torus", menu);

    append_menu_item(
        sub_menu, wxID_ANY, _L("Pyramid"), "", [type](wxCommandEvent&) { obj_list()->load_generic_subobject(L("Pyramid"), type); },
        "menu_obj_pyramid", menu);

    append_menu_item(
        sub_menu, wxID_ANY, _L("Prism"), "",
        [type](wxCommandEvent&) { obj_list()->load_generic_subobject(L("Prism"), type); }, "menu_obj_prism", menu);

    append_menu_item(
        sub_menu, wxID_ANY, _L("Disc"), "", [type](wxCommandEvent&) { obj_list()->load_generic_subobject(L("Disc"), type); },
        "menu_obj_disc", menu);
    ...
    std::vector<const char*> icon_names = {"Block20XY", "3DBenchy", "Complex",
                                            "Overhang", "Square columns Z axis", "Square prism Z axis"};
...
wxMenuItem* MenuFactory::append_menu_item_change_type(wxMenu* menu)
{
    ...
    assert(names.size() == ADD_VOLUME_MENU_ITEMS.size());

    for (size_t i = 0; i < names.size(); i++) {
        auto& name = names[i];
        
        /*append_menu_item(
            sub_menu, wxID_ANY, name, "", [i](wxCommandEvent&) { obj_list()->do_change_part_type(ModelVolumeType(i)); }, "", menu,
            [name]() {
                
                if ((name == _L("Support Blocker")) || (name == _L("Support Enforcer"))) {
                
                    ModelVolume* volume = obj_list()->get_selected_model_volume();
                    if (!volume)
                        return false;
                    return (!volume->is_svg() && !volume->is_text());
                }
        ...
        append_menu_check_item(
            sub_menu, wxID_ANY, name, "",
            [i](wxCommandEvent&) 
            {
                obj_list()->do_change_part_type(ModelVolumeType(i));
            },
            menu,
            [name]() 
            {
    ...
    return append_submenu(
        menu, sub_menu, wxID_ANY, _L("Change type"), "", "",
        []() {
            wxDataViewItem item = obj_list()->GetSelection();
            return (item.IsOk() || obj_list()->GetModel()->GetItemType(item) == itVolume) && obj_list()->can_change_part_type();
        },
        m_parent);
...
void MenuFactory::create_default_menu()
{
    ...
    wxMenu* sub_menu_handy = append_submenu_add_creality_model(&m_default_menu, ModelVolumeType::INVALID);*/

    wxMenu* sub_menu_testing_model = append_submenu_add_testing_model(&m_default_menu, ModelVolumeType::INVALID);
    ...
    append_submenu(
        &m_default_menu, sub_menu_testing_model, wxID_ANY, _L("Add testing Primitive"), "", "menu_add_part", []() { return true; }, m_parent);
    ...
    append_submenu(
        &m_default_menu, sub_menu_testing_model, wxID_ANY, _L("Add testing Primitive"), "", "", []() { return true; },
        m_parent);
...
void MenuFactory::create_plate_menu()
{
    ...
    wxMenu* sub_menu_handy = append_submenu_add_creality_model(menu, ModelVolumeType::INVALID);*/

    wxMenu* sub_menu_testing_model = append_submenu_add_testing_model(menu, ModelVolumeType::INVALID);
    ...
    append_submenu(
        menu, sub_menu_testing_model, wxID_ANY, _L("Add testing Primitive"), "", "menu_add_part", []() { return true; }, m_parent);
    ...
    append_submenu(
        menu, sub_menu_testing_model, wxID_ANY, _L("Add testing Primitive"), "", "", []() { return true; }, m_parent);
#endif
...
void MenuFactory::append_menu_item_change_filament(wxMenu* menu)
{
    ...
    ObjectDataViewModel* obj_model = list_model();
    bool isAllCon = std::all_of(sels.begin(), sels.end(), [obj_model](const auto& sel) {
        return obj_model->GetInfoItemType(sel) == InfoItemType::CutConnectors;
        });
    
    menu->Enable(2008, !isAllCon);
}
    ...
    auto item = append_menu_item(
        ...
        [plate](wxCommandEvent &e) {
            ...
            if (hover_idx == -1) {
                ...
                plater()->select_plate_by_hover_id(plate_idx * PartPlate::GRABBER_COUNT, false, false);
            }
            else
            {
